...
func WXLogin(code string) (*WxloginRsp, error) {
	// url组合完整请求
	url := fmt.Sprintf("https://api.weixin.qq.com/sns/jscode2session?appid=%s&secret=%s&js_code=%s&grant_type=authorization_code", config.Sysconfig.AppID, config.Sysconfig.SecretKeyWx, code)
	// 向微信发起请求
